...
use Omnipay\Ogone\Message\MoneticoPaiementEpt;
use Omnipay\Ogone\Message\MoneticoPaiementHmac;

class OgoneController extends Commerce_BaseFrontEndController
{
	...
	public function actionHandleReturn()
	{
		...
		if (!$transaction)
		{
			OgonePlugin::log("Couldn’t get transaction.", LogLevel::Error);
			$this->_returnFailure();
		}

		OgonePlugin::log("handleReturn called for order ID $transaction->orderId.", LogLevel::Info, true);
		...
		$phase2back_fields = sprintf(MONETICOPAIEMENT_PHASE2BACK_FIELDS,
			$data['TPE'],
			$data['date'],
			$data['montant'],
			$data['reference'],
		...
		if (strtolower($macToCompare) !== strtolower($mac))
		{
			...
			OgonePlugin::log("MAC didn’t match. Possible fraud attempt.", LogLevel::Info);
			$this->_returnFailure();
		}
		else
		{
			...
			if ($returnCode === 'payetest' || $returnCode === 'paiement')
			{
				...
				if (!$success)
				{
					OgonePlugin::log("Transaction was successful but couldn’t complete the order for order #".$transaction->order->number, LogLevel::Error);
				}
			}
			// Payment failed
			elseif ($returnCode === 'Annulation')
			{
				...
				if (!$success)
				{
					OgonePlugin::log("Transaction failed but we couldn’t update the transaction for id #".$transaction->id." on order #".$transaction->order->number, LogLevel::Error);
				}
			}
	...
	private function _returnFailure()
	{
		...
		ob_start();
		echo sprintf(MONETICOPAIEMENT_PHASE2BACK_RECEIPT, MONETICOPAIEMENT_PHASE2BACK_MACNOTOK);
		craft()->end();
	}
	...
	private function _returnSuccess()
	{
		HeaderHelper::setHeader(array('Content-Type' => 'text/plain'));
		ob_start();
		echo sprintf(MONETICOPAIEMENT_PHASE2BACK_RECEIPT, MONETICOPAIEMENT_PHASE2BACK_MACOK);
		craft()->end();
	}
